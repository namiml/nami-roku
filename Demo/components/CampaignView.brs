...
sub onItemSelected(event as dynamic)
    ...
    customAttributes = paywallLaunchContext.customAttributes
    if customAttributes = invalid
        customAttributes = createObject("roAssociativeArray")
    end if
    customAttributes.AddReplace("branchPath", "nobranch")
    paywallLaunchContext.customAttributes = customAttributes
